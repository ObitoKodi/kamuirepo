...
documented_source = os.path.join(profile, 'documented_source')
...
if os.path.exists(documented_source)==True:
    SOURCES = open(documented_source).read()
else: SOURCES = []
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(documented.hd, code error - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(documented.hd, server error. - "+str(e.reason)+",10000,"+icon+")")
				
def SKindex():
    addDir('[B][COLOR lime]| Favourites[/COLOR][/B]','[B][COLOR lime]| Favourites[/COLOR][/B]',4,'https://i.imgur.com/gQteu00.png',FANART,'','','','')
    getData(Base,FANART)
...
def getSources():
        try:
            if os.path.exists(favorites) == True:
                addDir('[B][COLOR lime]| Favourites[/COLOR][/B]','[B][COLOR lime]| Favourites[/COLOR][/B]',4,'https://i.imgur.com/gQteu00.png' , os.path.join(home, 'fanart.gif'),FANART,'','','','')
            if os.path.exists(documented_source)==True:
                sources = json.loads(open(documented_source,"r").read())
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':           
                ...
                contextMenu.append(('[B][COLOR blue]Remove Doc[COLOR white].[COLOR blue]HD Favourite[/COLOR][/B]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
            if not name in FAV:
                contextMenu.append(('[B][COLOR lime]Add to Doc[COLOR white].[COLOR lime]HD Favourites[/COLOR][/B]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('[B][COLOR blue]Remove Doc[COLOR white].[COLOR blue]HD Favourite[/COLOR][/B]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('[B][COLOR lime]Add to Doc[COLOR white].[COLOR lime]HD Favourites[/COLOR][/B]','XBMC.RunPlugin(%s)' %fav_params))                                                                                
            liz.addContextMenuItems(contextMenu)
        try:
...
params=get_params()
Base = 'http://frenchdj.atspace.tv/Documented/Documented.HD.xml'
url=None
name=None
mode=None
playlist=None
