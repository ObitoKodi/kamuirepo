...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.kamui.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.kamui.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.kamui.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.kamui.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.kamui.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.kamui.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
